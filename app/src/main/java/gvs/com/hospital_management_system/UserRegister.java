...
public class UserRegister extends AppCompatActivity {
    ...
    public void reg(View view){
        final String cpname=((EditText)findViewById(R.id.cname)).getText().toString();
        final String cemail=((EditText)findViewById(R.id.cemail)).getText().toString();
        final String cpphone=((EditText)findViewById(R.id.cphone)).getText().toString();
        final String cppass=((EditText)findViewById(R.id.cpass)).getText().toString();
        final String cpusername=((EditText)findViewById(R.id.cusername)).getText().toString();
        final String cusername_pass=cpusername+"_"+cppass;
        if (cpusername.length() >= 4 && cemail.length() >= 4 && cpphone.length() ==10 && cppass.length() >=4 && cpusername.length() >=4) {
            databaseReference = FirebaseDatabase.getInstance().getReference("User_Details");

            databaseReference.orderByChild("username").equalTo(cpusername).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if (dataSnapshot.exists()) {
                        ...
                        User_Register_Model fdel = new User_Register_Model(id, cpname, cemail, cpphone, cppass, cpusername, cusername_pass, username_otp, String.valueOf(x));
            ...
            Toast.makeText(UserRegister.this, " Please Fill The All Values above 4 Characters", Toast.LENGTH_SHORT).show();
